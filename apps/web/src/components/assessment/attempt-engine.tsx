...
export function AttemptEngine({ attemptId }: AttemptEngineProps) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 pb-28 pt-6 lg:pb-16 space-y-6">
      {/* ── Status Bar ── */}
      <section id="tour-attempt-status" className="sticky top-16 z-20 rounded-2xl border border-slate-200 bg-surface/95 backdrop-blur-md p-4 shadow-sm">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_19rem]">
        ...
        <section className="bg-surface border border-slate-200 rounded-3xl p-5 md:p-6 shadow-sm space-y-6">
                            ...
                            <button
                              className={`flex min-h-12 items-start gap-3 rounded-xl border p-2.5 text-left text-xs leading-relaxed transition-all ${
                                selected
                                  ...
                                  : "border-slate-200 bg-surface text-slate-700 hover:border-indigo-400"
                              }`}
                              key={`${key}-${optIdx}`}
                              onClick={(e) => {
                                e.stopPropagation();
                          ...
                          <div className="bg-slate-50/50 border border-slate-200 rounded-2xl p-5 space-y-4 animate-in fade-in duration-300">
                            ...
                            {savedMainsAns.checked_copy_url && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-surface px-3 py-2 text-xs font-black text-indigo-700 hover:bg-indigo-50"
                              >
                            ...
                            {savedMainsAns.feedback && (
                              <div className="space-y-1.5">
                                ...
                                <div
                                  ...
                                  className="prose prose-sm max-h-[240px] overflow-y-auto pr-1 text-xs leading-relaxed text-slate-700 space-y-2 border border-slate-200/60 p-3 bg-surface rounded-xl"
                                />
                              </div>
                            )}
                          </div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-surface p-5 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-5 gap-1.5">
              {paper.questions.map((question, index) => {
                ...
                let colorClass = "border-slate-200 bg-surface text-slate-700 hover:border-slate-400";

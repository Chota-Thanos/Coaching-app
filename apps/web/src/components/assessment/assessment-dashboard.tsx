                ...
                {categories.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {categories.map((name: string) => (
                      <span key={name} className="rounded-full bg-white px-2 py-0.5 text-[10px] font-bold text-slate-500">
                        {name}
                      </span>
                    ))}
                  </div>
                ...
                {activeTab === "mains" ? (
                  ...
                  mainsAnswers.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center">
                      ...
                      <p className="mt-2 text-sm font-bold text-slate-900">No Mains practice submissions yet.</p>
                      <p className="text-xs text-slate-500 mt-1 mb-4">Start drafting subjective answers and review detailed rubrics.</p>
                      <Link href="/assessment/mains" className="inline-flex h-10 items-center justify-center gap-1.5 rounded-xl bg-indigo-600 text-white font-bold text-xs px-4">
                        Browse Mains Questions
                      </Link>
                    </div>
                  ) : (
                    <div className="grid gap-3">
                      {mainsAnswers.map((attempt) => (
                        <div key={attempt.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
                          <div className="min-w-0 flex-1 space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className={`rounded px-2 py-0.5 text-[10px] font-bold ${
                                attempt.evaluation_status === "evaluated"
                                  ? "bg-green-50 text-green-700 border border-green-200"
                                  : attempt.evaluation_status === "ai_evaluating"
                              ...
                              {attempt.paper_name && (
                                <span className="text-[10px] font-extrabold text-indigo-600 bg-indigo-50 px-1.5 rounded uppercase">
                                  {attempt.paper_name}
                                </span>
                              )}
                            </div>
                            <h3 className="text-sm font-extrabold text-slate-900 leading-snug line-clamp-1">
                              {attempt.question_statement}
                            </h3>
                            {attempt.evaluation_status === "evaluated" && (
                              <p className="text-xs text-slate-500">
                                Evaluated score: <strong className="text-slate-800">{formatMarks(attempt.score)} / {formatMarks(attempt.max_score)}</strong> marks
                              </p>
                            )}
                          ...
                          <Link
                            href={
                              attempt.result_id
                                ? `/assessment/results/${attempt.result_id}`
                                : attempt.attempt_id
                                ? `/assessment/attempts/${attempt.attempt_id}`
                                : `/assessment/mains/${attempt.question_id ?? attempt.id}`
                            }
                            className="inline-flex h-9 items-center justify-center rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs px-4 shrink-0 transition-colors"
                          >
                            {attempt.evaluation_status === "evaluated" ? "Review Verdict" : "View Submission"}
                          </Link>
                        </div>
                      ))}
                    </div>
                  ...
                  recentAttempts.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center">
                      <p className="text-2xl">📝</p>
                      <p className="mt-2 text-sm font-bold text-slate-900">No attempts completed in this section.</p>
                      <button onClick={() => setActiveTab("gk")} className="mt-3 inline-flex h-10 items-center justify-center gap-1.5 rounded-xl bg-indigo-600 text-white font-bold text-xs px-4">
                        Practice MCQs
                      </button>
                    </div>
                  ) : (
                        ...
                        <div key={attempt.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
                          <div className="min-w-0 flex-1 space-y-1.5">
                            ...
                            <h3 className="text-sm font-extrabold text-slate-900 leading-snug">
                              {attempt.test_template?.title}
                            </h3>
                            {attempt.result && (
                              <div className="flex items-center gap-3 flex-wrap">
                                <span className="text-xs font-semibold text-slate-500">
                                  Score: <strong className="text-slate-800">{formatMarks(attempt.result.score)} / {formatMarks(attempt.result.max_score)}</strong> marks
                                </span>
                                <AccuracyPill value={attempt.result.accuracy} />
                              </div>
                            )}
                          </div>
                          ...
                          <div className="shrink-0 flex items-center">
                            {attempt.result ? (
                              <Link
                                href={`/assessment/results/${attempt.result.id}`}
                                className="inline-flex h-9 items-center justify-center rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs px-4"
                              >
                                Review Results
                              </Link>
                            ) : (
                              <Link
                                href={`/assessment/attempts/${attempt.id}`}
                                className="inline-flex h-9 items-center justify-center rounded-xl border border-amber-300 bg-amber-50 text-amber-800 font-bold text-xs px-4 hover:bg-amber-100"
                              >
                                Resume Test
                              </Link>
                            )}

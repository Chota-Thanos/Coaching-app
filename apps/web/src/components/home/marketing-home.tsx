...
import { PerformanceConsoleSection } from "../marketing/performance-console";
import {
  BookOpen,
  BookOpenCheck,
  ChevronRight,
 ...
 *
 * This used to live inside `app/page.tsx`, which was a client component that
 * returned a loading spinner until the auth context had read localStorage.
 * localStorage does not exist on the server, so that spinner was the *only*
 * thing ever server-rendered for `/` — search engines got 17KB of HTML with no
 * heading and no links, and the home page could not hold a ranking. Splitting
 * the marketing page out lets the server render all of it, with the signed-in
 * dashboard swapped in on the client afterwards. See `home-gate.tsx`.
 *
 * Its data now arrives as props, fetched on the server by the page, so the
 * article and mentor cards are in the HTML too, rather than appearing after
 * three client-side round trips.
 */
...
export function MarketingHome({ latestArticles, mentors, diagnosticTestId }: MarketingHomeProps) {
  // Presentation-only state for the "pick your topics" test-builder preview in
  // the modules section. Nothing here is fetched or persisted.

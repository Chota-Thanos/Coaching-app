...
- [x] Ported the mobile typography/design work to the web Study Plans pages (`apps/web/src/app/study-plans/page.tsx`, `apps/web/src/components/study-plans/study-plan-detail-client.tsx`), per explicit follow-up request to match two reference screenshots' type treatment and bring the web pages to parity with the mobile redesign. Found and fixed a real bug along the way, scoped to just these two files rather than sitewide: global CSS (`apps/web/src/app/globals.css`) forces `h1-h6{font-weight:600!important}` and `button{font-family:Inter!important}`, silently overriding every bold-heading/Plus-Jakarta-Sans Tailwind class already written in these components. Fixed with Tailwind's `!` important-modifier (e.g. `!font-black`, `!font-heading`) applied locally, confirmed via computed-style inspection in a live dev server (`fontWeight: "900"`, `fontFamily` including `"Plus Jakarta Sans"`) rather than touching the global rule, which would have changed heading weight sitewide. Catalog cards (`page.tsx`) now overlay a level pill + free/paid badge on cover art, matching the mobile card's visual language. Detail page curriculum section replaced the flat per-week list with a roadmap presentation -- connector line, done/current/locked status circles, collapsible weeks that auto-expand only the current in-progress week, strikethrough on completed day items -- porting the structure from `study_plan_detail_screen.dart`'s `_buildWeekNode`/`_buildDayRow`. Purchase panel restyled as a compact pricing card (eyebrow label, bold price, prominent CTA). Verified with `tsc --noEmit` and `next build`, both clean. Deployed to production the same day; see `docs/deployment.md` for the deploy log and a real gotcha hit along the way (Windows SSH clients prompting for a local key's passphrase instead of the server password).
